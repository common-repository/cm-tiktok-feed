...
if( !class_exists('Wbcr_FactoryPages430_AdminPage') ) {

	class Wbcr_FactoryPages430_AdminPage extends Wbcr_FactoryPages430_Page {
		...
		public function connect()
		{
			...
			$this->hidden = apply_filters('wbcr_factory_430_page_is_hidden_' . $result_id, $this->hidden);

			if( $this->hidden ) {
				return;
			}

			$this->internal = apply_filters('wbcr_factory_430_page_is_internal_' . $result_id, $this->internal);
			...
			if( $this->request->get('page', 'none') == $result_id ) {
				...
				if( !$this->scripts->isEmpty('bootstrap') || !$this->styles->isEmpty('bootstrap') ) {
					add_action('wbcr_factory_430_bootstrap_enqueue_scripts_' . $this->plugin->getPluginName(), [
						$this,
