...
if( !class_exists('Wbcr_FactoryPages430_ImpressiveThemplate') ) {
	/**
	 * Class Wbcr_FactoryPages430_ImpressiveThemplate
	 */
	abstract class Wbcr_FactoryPages430_ImpressiveThemplate extends Wbcr_FactoryPages430_AdminPage {
		 ...
		 */
		public function __construct(Wbcr_Factory430_Plugin $plugin)
		{
			$this->menuIcon = FACTORY_PAGES_430_URL . '/templates/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_430_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __('Settings', 'wbcr_factory_pages_430');
		...
		public function assets($scripts, $styles)
		{
			...
			$this->styles->add(FACTORY_PAGES_430_URL . '/templates/assets/css/impressive.page.template.css');
		}
		...
		public function getPluginTitle()
		{
			 ...
			 */
			$plugin_title = wbcr_factory_430_apply_filters_deprecated('wbcr/factory/imppage/plugin_title', [
		...
		public function flushCacheAndRulesAction()
		{
			 ...
			 */
			wbcr_factory_430_do_action_deprecated('wbcr_factory_430_imppage_flush_cache', [
				$this->plugin->getPluginName(),
				$this->getResultId()
			], '4.0.1', 'wbcr_factory_430_imppage_after_form_save');

			/**
			 * @since 4.0.9 - является устаревшим
			 */
			wbcr_factory_430_do_action_deprecated('wbcr_factory_430_imppage_after_form_save', [
				$this->plugin,
				$this
			 ...
			 * @since 4.0.9 - является устаревшим
			 */
			$redirect_args = wbcr_factory_430_apply_filters_deprecated('wbcr_factory_430_imppage_after_form_save_redirect_args', [$redirect_args], '4.0.9', 'wbcr/factory/pages/impressive/save_redirect_args');
		...
		{
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_430'));
			}*/
		...
		protected function printAllNotices()
		{
			 ...
			 * @since 4.0.9 - является устаревшим
			 */
			wbcr_factory_430_do_action_deprecated('wbcr_factory_pages_430_imppage_print_all_notices', [
				$this->plugin,
				$this
		...
		private function showActionsNotice()
		{
			$notices = [
				[
					...
					'type' => 'success',
					'message' => __('The settings have been updated successfully!', 'wbcr_factory_pages_430') . (WP_CACHE ? '<br>' . __("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_430') : '')
				]
			];
			 ...
			 * @since 4.0.9 - является устаревшим
			 */
			$notices = wbcr_factory_430_apply_filters_deprecated('wbcr_factory_pages_430_imppage_actions_notice', [
				$notices,
				$this->plugin,
				$this->id
				...
				<div class="wbcr-factory-control">
					...
					<?php if( $this->type == 'options' ): ?>
						<input name="<?= $this->plugin->getPluginName() ?>_save_action"
						       ...
						       value="<?php _e('Save', 'wbcr_factory_pages_430'); ?>">
		...
		protected function getPageWidgets($position = 'bottom')
		{
			 ...
			 * @since 4.0.9 - является устаревшим
			 */
			$widgets = wbcr_factory_430_apply_filters_deprecated('wbcr_factory_pages_430_imppage_get_widgets', [
				$widgets,
				$position,
				$this->plugin,
				$this
		...
		{
			$form = new Wbcr_FactoryForms428_Form([
				'scope' => rtrim($this->plugin->getPrefix(), '_'),
				'name' => $this->getResultId() . "-options"
			], $this->plugin);

			$form->setProvider(new Wbcr_FactoryForms428_OptionsValueProvider($this->plugin));
			...
			if( isset($_POST[$this->plugin->getPluginName() . '_save_action']) ) {
				...
				if( !current_user_can('administrator') && !current_user_can($this->capabilitiy) ) {
					wp_die(__('You do not have permission to edit page.', 'wbcr_factory_pages_430'));
					exit;
				}
				 ...
				 * @since 4.0.9 - является устаревшим
				 */
				wbcr_factory_430_do_action_deprecated('wbcr_factory_430_imppage_before_form_save', [
					$form,
					$this->plugin,
					$this
				 ...
				 * @since 4.0.9 - является устаревшим
				 */
				wbcr_factory_430_do_action_deprecated('wbcr_factory_430_imppage_form_saved', [
					$form,
					$this->plugin,
					$this
			...
			<div id="WBCR" class="wrap">
				<div class="wbcr-factory-pages-430-impressive-page-template factory-bootstrap-431 factory-fontawesome-000">
