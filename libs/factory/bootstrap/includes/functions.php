 ...
 * @since 3.2.0
 */
class Wbcr_FactoryBootstrap431_Manager {
	...
	public function loadAssets( $hook ) {
		...
		do_action( 'wbcr_factory_430_bootstrap_enqueue_scripts', $hook );
		do_action( 'wbcr_factory_430_bootstrap_enqueue_scripts_' . $this->plugin->getPluginName(), $hook );
	...
	protected function enqueueScripts( array $scripts, $type, array $dependencies ) {
		...
		$cache_dir_path = FACTORY_BOOTSTRAP_431_DIR . '/assets/cache/';
		$cache_dir_url  = FACTORY_BOOTSTRAP_431_URL . '/assets/cache/';
		...
		if ( $cache_enable && file_exists( $cache_filepath ) ) {
			...
			foreach ( $scripts as $script_to_load ) {
				...
				if ( $cache_enable && $cache_dir_exists ) {
					$fname = FACTORY_BOOTSTRAP_431_DIR . "/assets/$type-min/$script_to_load.min." . $type;
					...
					if ( $type == 'js' ) {
						wp_enqueue_script( md5( $script_to_load ), FACTORY_BOOTSTRAP_431_URL . "/assets/$type-min/$script_to_load.min." . $type, $is_first ? $dependencies : false, $this->plugin->getPluginVersion() );
					} else {
						wp_enqueue_style( md5( $script_to_load ), FACTORY_BOOTSTRAP_431_URL . "/assets/$type-min/$script_to_load.min." . $type, [], $this->plugin->getPluginVersion() );
					}
					$is_first = false;
				}
			}
