<?php
	...
	if( !class_exists('Wbcr_FactoryForms428_CheckboxControl') ) {

		class Wbcr_FactoryForms428_CheckboxControl extends Wbcr_FactoryForms428_Control {
			...
			protected function buttonsHtml()
			{
				...
				<div <?php $this->attrs() ?>>
					<button type="button" class="btn btn-default btn-small btn-sm factory-on <?php if( $value ) {
						echo 'active';
					} ?>"><?php _e('On', 'wbcr_factory_forms_428') ?></button>
					<button type="button" class="btn btn-default btn-small btn-sm factory-off <?php if( !$value ) {
						echo 'active';
					} ?>" data-value="0"><?php _e('Off', 'wbcr_factory_forms_428') ?></button>
