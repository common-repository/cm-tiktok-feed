<?php
	...
	if ( ! class_exists( 'Wbcr_FactoryForms428_DropdownControl' ) ) {
		
		class Wbcr_FactoryForms428_DropdownControl extends Wbcr_FactoryForms428_Control {
				...
				if ( $this->isAjax() ) {
                    ...
                    <script>
						window['<?php echo $ajax_id ?>'] = {
							...
							'empty_list': '<?php echo $this->getOption( 'empty', __( 'The list is empty.', 'wbcr_factory_forms_428' ) ) ?>'
						};
                    </script>
					<?php
			...
			protected function defaultHtml() {
				...
				$empty_list = $this->getOption( 'empty', __( '- empty -', 'wbcr_factory_forms_428' ) );
			...
			protected function printItems( $items, $selected = null ) {
				...
				foreach ( (array) $items as $item ) {
					...
					if ( isset( $item['type'] ) || isset( $item['value'] ) ) {
						...
						$title = isset( $item['title'] ) ? $item['title'] : __( '- empty -', 'wbcr_factory_forms_428' );
