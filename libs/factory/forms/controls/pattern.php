<?php
	...
	if( !class_exists('Wbcr_FactoryForms428_PatternControl') ) {

		class Wbcr_FactoryForms428_PatternControl extends Wbcr_FactoryForms428_Control {
			...
			public function __construct($options, $form, $provider = null)
			{
				...
				$this->patterns = apply_filters('wbcr_factory_forms_428_patterns', array());
				$this->patterns = apply_filters('wbcr_factory_forms_428_patterns-' . $name, $this->patterns);

				$this->custom_patterns = $this->getOption('patterns', array());

				$this->color = new Wbcr_FactoryForms428_ColorControl($options['color'], $form, $provider);
			}
				...
				<div <?php $this->attrs() ?>>
					<div class="factory-pattern-controls">
						...
						<a href="#" class="button button-default factory-button factory-change-color-btn <?php if( $hasColor ) {
							echo 'button-active';
						} ?>" title="<?php _e('Change color', 'wbcr_factory_forms_428') ?>">
							<i class="fa fa-flask"></i>
							<span><?php _e('re-color', 'wbcr_factory_forms_428') ?></span>
						</a>
					...
					<div class="factory-color-panel">
						<div class="factory-color-wrap">
							<span class="factory-color-label"><?php _e('Select color:', 'wbcr_factory_forms_428') ?></span>
							<?php $this->color->html() ?>
							<div class="factory-hint">
								<i><?php _e('Changing the color may takes a minute or more. Please be patient.', 'wbcr_factory_forms_428') ?></i>
							</div>
						</div>
