<?php
	 ...
	 * @since 1.0.0
	 */
	class Wbcr_FactoryForms428_GoogleFontControl extends Wbcr_FactoryForms428_FontControl {
		...
		public function __construct($options, $form, $provider = null)
		{
			...
			$this->google_font_data = new Wbcr_FactoryForms428_HiddenControl($option_google_font_data, $form, $provider);
		...
		public function getDefaultFonts()
		{
			...
			$fontsCommon = array(
				'group',
				__('Standard:', 'wbcr_factory_forms_428'),
				array(
			...
			$fontsGoogleFonts = array('group', __('Google Fonts:', 'wbcr_factory_forms_428'), array());
		...
		protected function getGoogleFonts()
		{
			...
			if( is_wp_error($response) ) {
				...
				$this->error = __('Unable to retrieve the list of Google Fonts.', 'wbcr_factory_forms_428');
			...
			if( !isset($response['body']) ) {
				...
				$this->error = __('Invalide response from the Google Fonts API.', 'wbcr_factory_forms_428');
			...
			if( empty($body->items) ) {
				...
				$this->error = __('Unexpected error. The list of Google Fonts are empty.', 'wbcr_factory_forms_428');
