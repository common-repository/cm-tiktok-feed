<?php
	...
	if( !class_exists('Wbcr_FactoryForms428_ListControl') ) {

		class Wbcr_FactoryForms428_ListControl extends Wbcr_FactoryForms428_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo $ajax_id ?>'] = {
							...
							'emptyList': '<?php echo $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_428') ) ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_428'));
